      ...
      <van-tabs :active="active" @click="tabClick" :swipe-threshold=5>
          ...
          <div>
            <van-list v-model="loading" :finished="finished" :immediate-check="immediateCheck"  @load="onLoad0('0')">
              <div class="curHoldCon">
                <p class="curHold"><span>合约/方向</span><span>总保证金</span><span>浮动盈亏</span><span>操作</span></p>
                <van-cell v-for="(item,index) in conListInfos0" :key="index">
                  <ul class="curHoldInfos" :class="item.fdyk>0?'red':'green'">
                    <li >
                      <span>{{(+item.eachNumber).toFixed(1)}}{{item.symbol.split('/')[0]}} / {{item.direction==0?'涨':'跌'}}</span>
                      <span>{{item.orderPrice}} {{item.symbol.split('/')[1]}}</span>
                      <span>{{(+item.fdyk).toFixed(5)}} {{item.symbol.split('/')[1]}}</span>
        ...
        <van-tab v-for="(item,index) of titData" :key="index">
          ...
          <div class="curOrderCon">
            <p class="curOrder"><span>时间</span><span>方向</span><span>金额</span><span>盈亏</span><span>操作</span></p>
            <van-list v-model="loading1" :finished="finished" :immediate-check="immediateCheck"  @load="onLoad1(index+1)">
              <van-cell v-for="(list,index) in conListInfos1" :key="index">
                ...
                <ul :class="{active1: curNum == index}" class="detailInfos">
                  <li>
                    <p><span>建仓时间：{{list.buyTime}}</span><span>{{list.symbol}}</span></p>
                    <p><span>手续费：{{list.fee}}</span><span>建仓价：{{list.buyPoint}}</span></p>
                    <p><span>盈亏: {{list.difMoney}}</span><span>合约/方向:{{(+list.eachNumber).toFixed(1)}}{{list.symbol.split('/')[0]}} /{{list.direction == '0'?'买涨':list.direction == '1'?'买跌':'无效方向'}}</span></p>
            ...
            if(res.data.errorCode === 0){
              this.conListInfos0.forEach((item,index)=> {
                ...
                if(item.direction== '0'){//当前价 - 建仓价 （买涨）
                  ...
                  // this.$set(item , 'fdyk', Number(item.hands*((item.latestPrice) - (item.buyPoint) )* item.pointPrice));
                  ...
                  this.$set(item , 'fdyk', Number(item.hands*((item.buyPoint) - (item.latestPrice) )* item.pointPrice));
                }
              })
            }else {
              this.$toast(res.data.errorMsg);
...
<style scoped>
  ...
  .detailInfos li p:first-child span:first-child {
    display: inline-block;
    width: 60%;
  }
  .detailInfos li p:first-child span:last-child {
    display: inline-block;
    width: 40%;
  }
