        ...
        if (psw.length < 4) {
          ...
          if (this.pswObj.step == 1) {
            if (this.checkPass(this.pswObj.fpassword, psw)) {
              ...
              this.$refs.gestureTitle.style.color = "#2CFF26";
              var str = '';
              psw.forEach((item)=>{
                str += item.index;
              })
              this.$axios({
                url: '/api/user/open/pattern',
                method: 'post',
                data: {
                  patternPassword: str,
                }
                ...
                if(!res.data.errorCode) {
                  this.$toast.success('密码保存成功');
                  localStorage.setItem('userMob', this.b64DecodeUnicode(this.userData.mobile)) //手势登录手机号
                  localStorage.setItem('loginType', 1) //登录方式
                  this.$back();
                }
              })
            } else {
